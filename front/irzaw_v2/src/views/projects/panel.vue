            ...
            <div class="button-areas">
                <div class="top-button">
                    <div class="" @click="this.card2 = !this.card2">
                        <router-link to="/newmultiple">Tag Mode</router-link>
                    </div>
                </div>

                <div class="top-button">
                    <div class="">
                        <router-link to="/newadd">Add Project</router-link>
                    </div>
                </div>
        ...
        <div class="row">
            <div class='search input-group' id="t">
                ...
                <span class="input-group-label" id="dark">
                    <router-link to="/listv5" id="white">Edit Filters</router-link>
                </span>
            ...
            <draggable :list="draggableValue"  :options="{}" tag="tbody" @change="onChange">
                <template #item="{element}">
                    <tr v-show="element.name.toString().toLowerCase().includes(this.name.toString().toLowerCase()) || element.project_number.toString().toLowerCase().includes(this.name.toString().toLowerCase()) || element.location.toString().toLowerCase().includes(this.name.toString().toLowerCase()) || element.client.toString().toLowerCase().includes(this.name.toString().toLowerCase()) || element.category.toString().toLowerCase().includes(this.name.toString().toLowerCase()) || element.description.toString().toLowerCase().includes(this.name.toString().toLowerCase())">
                        ...
                        <td>
                            <router-link :to="{name: 'editor', params:{select: element.id}, query: {page: 'Info'}}">Edit</router-link>
                        </td>
                        <td><a class="delete" @click="deleteProject(element.id)">Delete</a></td>
                        <td>
                            <router-link :to="{name: 'editor', params:{select: element.id}, query: {page: 'Preview'} }">Preview</router-link>
                        </td>
                    </tr>
                </template>
            </draggable>
...
import header1 from '@/components/header/main_header.vue';
//import listv from '@/views/listv4.vue';
import checkboxes from '@/components/checkbox/checkbox3.vue';
//import icon from 'v-svg-icons'
...
export default {
    ...
    methods: {
        ...
        setNewValue(id, event) {
            ...
            axios.post('https://irzaw.com/active_project', variable)
        ...
        deleteProject(id) {
            ...
            console.log(variable);
            axios.post('https://irzaw.com/delete_project', variable)
                .then(this.getdata());
            this.getdata();
        ...
        getdata() {
            let self = this;
            axios.get('https://irzaw.com/getProjects').then(function (response) {
                console.log(response);
                self.projects = response.data;
            });
            axios
                .get('https://mpengs.nyc/get_topfilters')
                .then(response => (this.current_filters = response.data))
            axios.get('https://irzaw.com//get_allTags').then(function (response) {
                console.log(response);
